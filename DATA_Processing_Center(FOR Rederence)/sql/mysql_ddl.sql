...
CREATE TABLE IF NOT EXISTS ods_battle_log (
    ...
    tower_score         FLOAT       COMMENT '防守综合评分',    is_narrow_win       TINYINT     COMMENT '险胜标记：胜利且基地血量<20%（新增）',
    is_first_attempt    TINYINT     COMMENT '首次挑战标记：用户第一次尝试该地图（新增）',    dt                  VARCHAR(10) COMMENT '数据日期 yyyy-MM-dd（原Hive分区字段）'
...
CREATE TABLE IF NOT EXISTS dws_user_battle_stats (
    ...
    -- 新增业务洞察特征
    narrow_win_count            INT    COMMENT '险胜场次（胜利且基地血量<20%）（新增）',
    narrow_win_rate             FLOAT  COMMENT '险胜率（险胜数/总场次）（新增）',
    first_attempt_win_rate      FLOAT  COMMENT '首次尝试胜率（衡量初始难度适应性）（新增）',
    retry_map_count             INT    COMMENT '重试过的地图数（衡量坚持程度）（新增）',
    progress_velocity           FLOAT  COMMENT '关卡推进速率=最高关卡/观测天数（新增）'
...
CREATE TABLE IF NOT EXISTS dws_user_d1_stats (
    ...
    d1_unique_maps        INT    COMMENT '首日探索关卡数（初始参与深度）（新增）'
...
CREATE TABLE IF NOT EXISTS ads_daily_churn_summary (
    ...
    stuck_user_count     INT         COMMENT '居円用户数即stuck_level_count>0（新增）',
...
CREATE TABLE IF NOT EXISTS ads_map_churn_hotspot (
    map_id                  INT         COMMENT '地图 ID',
    difficulty_tier         VARCHAR(20) COMMENT '难度分级：easy/normal/hard/extreme',
    total_attempts          INT         COMMENT '总尝试场次',
    fail_rate               FLOAT       COMMENT '失败率（1-通关率）',
    avg_hp_ratio            FLOAT       COMMENT '平均基地剩余血量比例',
    help_usage_rate         FLOAT       COMMENT '特殊塔/道具使用率',
    player_count            INT         COMMENT '玄地玩家数',
