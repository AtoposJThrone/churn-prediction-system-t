-- =============================================================
-- hive_ddl.sql
-- 塔防游戏流失预警系统 —— Hive 数仓分层建表语句
-- 分层：ODS（原始） → DWD（明细） → DWS（汇总） → ADS（应用）
-- 分区字段统一使用 dt（yyyy-MM-dd），便于增量调度
-- =============================================================
...
CREATE DATABASE IF NOT EXISTS td_churn
  COMMENT '塔防游戏流失预警数仓'
  LOCATION '/user/hive/warehouse/td_churn.db';
...
CREATE EXTERNAL TABLE IF NOT EXISTS ods_battle_log (
    ...
    battle_start_time   STRING     COMMENT '战斗开始时间（字符串原样存储）',
...
STORED AS TEXTFILE
LOCATION '/user/hive/warehouse/td_churn.db/ods_battle_log'
TBLPROPERTIES ('skip.header.line.count'='1');

-- ODS: 地图元数据表（全量，无分区）
CREATE EXTERNAL TABLE IF NOT EXISTS ods_map_meta (
    map_id                  INT     COMMENT '地图ID',
    ...
    difficulty_tier         STRING  COMMENT '难度分级',
...
FIELDS TERMINATED BY ','
STORED AS TEXTFILE
LOCATION '/user/hive/warehouse/td_churn.db/ods_map_meta'
TBLPROPERTIES ('skip.header.line.count'='1');

-- ODS: 用户标签表（train/dev/test 合并后导入）
CREATE EXTERNAL TABLE IF NOT EXISTS ods_user_label (
    user_id   INT      COMMENT '用户ID',
    label     TINYINT  COMMENT '流失标签：1=流失，0=留存，NULL=测试集',
    split     STRING   COMMENT '数据集划分：train/dev/test'
)
COMMENT 'ODS层-用户流失标签表'
ROW FORMAT DELIMITED
FIELDS TERMINATED BY ','
STORED AS TEXTFILE
LOCATION '/user/hive/warehouse/td_churn.db/ods_user_label'
TBLPROPERTIES ('skip.header.line.count'='1');
...
CREATE TABLE IF NOT EXISTS dwd_battle_detail (
    ...
    battle_date           STRING  COMMENT '战斗日期 yyyy-MM-dd',
    session_gap_s         FLOAT   COMMENT '与上一场间隔（秒）',
    session_id            STRING  COMMENT '会话ID（user_id + 会话序号）',
    cumulative_battles    INT     COMMENT '累计战斗场次',
    consecutive_fail      INT     COMMENT '当前连续失败次数',
...
CREATE TABLE IF NOT EXISTS dwd_session_detail (
    ...
    session_date         STRING COMMENT '会话日期',
...
CREATE TABLE IF NOT EXISTS ads_user_churn_risk (
    ...
    rule_trigger        STRING  COMMENT '命中的规则列表（JSON字符串）',
    final_alert         TINYINT COMMENT '最终预警结果：1=预警，0=安全',
    top_reason_1        STRING  COMMENT '主要流失原因1',
    top_reason_2        STRING  COMMENT '主要流失原因2',
    top_reason_3        STRING  COMMENT '主要流失原因3',
    predict_dt          STRING  COMMENT '预测日期',
    model_version       STRING  COMMENT '使用的模型版本'
)
...
-- =============================================================
-- 数据加载示例（Spark 写入 Hive 后手动触发分区修复）
-- =============================================================
